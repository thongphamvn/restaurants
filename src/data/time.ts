...
const buildSearchTimes = (time: string) => {
  const searchTimes = []
  const length = Object.keys(displayTimeObject).length
  const timeIndex = Object.keys(displayTimeObject).indexOf(time)
  const startIndex = timeIndex - 2 > 0 ? timeIndex - 2 : 0
  const endIndex = timeIndex + 2 < length ? timeIndex + 2 : length

  for (let i = startIndex; i < endIndex; i++) {
    searchTimes.push(Object.keys(displayTimeObject)[i])
  }
  return searchTimes
}

export const times = Object.keys(displayTimeObject).map((time) => {
  return {
    time,
    displayTime: displayTimeObject[time as keyof typeof displayTimeObject],
    searchTimes: buildSearchTimes(time),
  }
})
